      ...
      <table class="uk-table uk-table-small uk-table-divider uk-table-middle table-rows">
        ...
        <tbody>
          <tr v-for="bank in banklist.results">
            ...
            <td>
              <img class="uk-width-1-6" :src="url.replace('/cp/management', '') + '/images/bankpayment/' + bank.bank_logo" :alt="bank.bank_name">
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
      ...
      else
      {
        if( this.forms.edit === false )
        {
          ...
          else
          {
            ...
            axios.post( this.url + '/bankpayment/add', formfile).then( res => {
              swal({
                title: 'Berhasil',
        ...
        else
        {
          ...
          formfile.append('bank_name', this.forms.name);
          formfile.append('bank_logo', this.forms.logo, this.forms.logo.name);
          axios.post( this.url + '/bankpayment/update/' + this.forms.id, formfile).then( res => {
            swal({
              title: 'Berhasil',
              text: res.data.statusText,
              icon: 'success'
    ...
    deleteBankList(id) {
      swal({
        ...
        if( res )
        {
          axios({
            method: 'delete',
            url: this.url + '/bankpayment/delete/' + id
          }).then( res => {
            swal({
              title: 'Berhasil',
              text: res.data.statusText,
    ...
    getBankList(pages) {
      ...
      if( pages === undefined )
        pages = this.url + '/bankpayment/banklist?page=' + this.pagination.current + params;
      else
        pages = pages + params;
