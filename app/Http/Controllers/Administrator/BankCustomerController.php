...
class BankCustomerController extends Controller
{
  public function index( Request $request )
  {
    ...
    {
      return response()->view('administrator.pages.bankcustomer', [
        'request' => $request,
  ...
  public function banklist( Request $request, BankCustomer $bank )
  {
  ...
  public function store( Request $request, BankCustomer $bank )
  {
    ...
    else
    {
      ...
      else
      {
        ...
        $query->save();
        Storage::disk('imagestorage')->putFileAs( 'bankcustomer', $banklogo, $getfilename );

        $res = [
          'status' => 200,
          'statusText' => 'Bank customer berhasil ditambah',
          'size' => $getsize
        ];
      }
    }
  ...
  public function update( Request $request, BankCustomer $bank, $id )
  {
    $bankcode = $request->bank_code;
    $bankname = $request->bank_name;
    ...
    if( $bankcode == $getbank->bank_code )
    {
      ...
      if( ! empty( $banklogo ) )
      {
        ...
        {
          if( Storage::disk('imagestorage')->exists('bankcustomer/' . $getbank->bank_logo) )
          {
            Storage::disk('imagestorage')->delete('bankcustomer/' . $getbank->bank_logo);
          }
          Storage::disk('imagestorage')->putFileAs( 'bankcustomer', $banklogo, $getfilename );
          ...
          $res = [
            'status' => 200,
            'statusText' => 'Bank customer berhasil diupdate'
          ];
        }
      }
      else
  ...
  public function destroy( BankCustomer $bank, $id )
  {
